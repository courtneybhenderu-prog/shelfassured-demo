-- Check stores for DJ's Boudain brand
...
-- Step 2: Count stores for DJ's Boudain (replace BRAND_ID with actual ID from step 1)
-- First, let's do it dynamically by finding the brand first
WITH brand_info AS (
    SELECT id, name
    FROM brands
    WHERE LOWER(name) LIKE '%dj%' 
...
-- Step 3: List all stores for DJ's Boudain with details
WITH brand_info AS (
    SELECT id, name
    FROM brands
    WHERE LOWER(name) LIKE '%dj%' 

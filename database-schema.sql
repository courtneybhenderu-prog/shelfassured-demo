...
CREATE TABLE jobs (
    ...
    total_payout DECIMAL(10,2) GENERATED ALWAYS AS (
        CASE 
            WHEN all_stores THEN payout_per_store * (SELECT COUNT(*) FROM stores WHERE is_active = true)
            ELSE payout_per_store * (SELECT COUNT(*) FROM job_stores WHERE job_id = id)
        END
    ) STORED,

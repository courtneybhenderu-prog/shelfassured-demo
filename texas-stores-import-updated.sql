-- Updated Texas Stores Import Script (Using Google Sheet Column Mapping)
-- This script imports the curated Texas store data using the correct column mapping:
-- Column E (STORE) -> name field
-- Columns G+H+I+J (ADDRESS+CITY+STATE+ZIP) -> address components
-- All other columns preserved for future use
...
-- Step 2: Create temporary table for import with correct column mapping
...
-- Step 3: Insert sample Texas stores with correct column mapping
...
-- Step 5: Insert into main stores table with correct mapping
INSERT INTO public.stores (
    ...
    chain,           -- Column A (CHAIN)
    banner,          -- Column C (BANNER)
    store_number,    -- Column G (Store #)
    phone,           -- Column M (PHONE)
    metro,           -- Column L (METRO)
    country,
    source,          -- Flag as imported
    is_active,
    created_at,
    updated_at
)
SELECT
    ...
    tts.chain,                    -- Column A: Chain
    tts.banner,                   -- Column C: Banner
    tts.store_number,             -- Column G: Store number
    tts.phone,                    -- Column M: Phone
    tts.metro,                    -- Column L: Metro area
    'US',                         -- Country
    'imported',                   -- Source flag
    TRUE,                         -- Active
...
-- Verify final count and source distribution
SELECT 
    source,
